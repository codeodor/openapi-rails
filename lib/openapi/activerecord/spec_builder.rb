module Openapi
  class SwaggerRoot
    ...
    def self.build_specification(config, controller_classes)
      swagger_root do
        key :swagger,  '2.0'
        key :host,     ENV['HOST'] || 'localhost:3001'
    ...
    module SpecBuilder
      ...
      class_methods do
        ...
        def build_openapi_specification(options)
          ...
          self.spec_relative_path ||= routes.first[1].
          ...
          puts "Building specs for #{self.spec_resource_name}"
          puts self.spec_resource_name

          self.spec_collection_name ||=
            self.spec_resource_name.pluralize

          puts self.spec_collection_name

          build_openapi_definitions
          build_openapi_paths(routes)
        end

        def build_openapi_paths(routes = Openapi::RoutesParser.new(self).routes)
        ...
        def build_openapi_definitions
          ...
          puts "building definitions: "
          puts resource_name
          puts resource_class
          swagger_schema resource_name do
            build_model_schema(resource_class)

            resource_class.reflections.each do |key, reflection|
              relation = reflection
              relation_type = reflection.class.name
